...
`ifdef FPGA
    initial begin
        $display("[L0DIAG][FATAL] FPGA macro enabled.");
        $finish;
    end
`else
    ...
    initial begin
        ...
        $display("[L0DIAG] Loading DRAM .mem files...");
        ...
        $display("[L0DIAG] DRAM[0]=%08x  IFM[%0d]=%08x  GOLDEN[0]=%02x",
        ...
        $display("[L0DIAG][%0t] ap_start sent", $time);

        // 6) L0 완료 대기 (layer_idx 가 0 → 1 로 전이)
        wait (u_dut.layer_idx == 5'd1);
        $display("[L0DIAG][%0t] L0 done (layer_idx -> 1)", $time);
        #(40*PERIOD);
                            ...
                            for (sub_w = 0; sub_w < 2; sub_w = sub_w + 1) begin
                                ...
                                if (got !== exp) begin
                                    ...
                                    if (mm_print_cnt < 8) begin
                                        $display("  [L0DIAG] MISMATCH fi=%0d h=%0d w=%0d got=%02x exp=%02x (d=%0d)",
                                            fi, h, w, got, exp, diff);
    ...
    task report_stats;
        ...
        begin
            ...
            $display("[L0DIAG] delta dist : total=%0d  +d=%0d  -d=%0d  max|d|=%0d",
                     mm_total, cnt_pos, cnt_neg, max_diff);
            $display("[L0DIAG]   |d|=1    : %0d  (%0d%%)",
                     cnt_d1, (cnt_d1*100)/((mm_total==0)?1:mm_total));
            $display("[L0DIAG]   |d|=2    : %0d", cnt_d2);
            $display("[L0DIAG]   |d|=3    : %0d", cnt_d3);
            $display("[L0DIAG]   |d|=4..8 : %0d", cnt_d4_8);
            $display("[L0DIAG]   |d|>8    : %0d", cnt_d_big);
            $display("[L0DIAG]   |d|>%0d (tol-exceed) : %0d", TOLERANCE, mm_tol_fail);

            $display("[L0DIAG][Result] OFM mismatch: %0d / 1048576   (tol-exceed: %0d)", mm_total, mm_tol_fail);
            if      (mm_total == 0)    $display("[L0DIAG][Result] *** PASS (exact, 0 mismatches) ***");
            else if (mm_tol_fail == 0) $display("[L0DIAG][Result] *** PASS (within tolerance +-%0d, %0d noise) ***", TOLERANCE, mm_total);
            else                       $display("[L0DIAG][Result] *** FAIL (%0d exceed +-%0d tolerance) ***", mm_tol_fail, TOLERANCE);

            $display("");
            $display("[L0DIAG] ============================================================");
            $display("[L0DIAG] L0 : %s (mismatch=%0d, tol-exceed=%0d)",
    ...
    initial begin
        ...
        $display("[L0DIAG] *** TIMEOUT (2 sec sim time) ***");
        $finish;
    end

    // ================================================================
    //  conv 정렬 진단 trace (look-ahead / read-latency 검증용)
    //    발사 주소(o_ifm_*) → IFM 도착(ifm_00) → mac_vld_d → pixel_vld 의
    //    cycle 정렬을 기록. B(파이프라인) 전/후 로그를 비교해 정렬 유지 확인.
    //    cstate: 0=IDLE 1=LOAD 2=RUN 3=DRAIN 4=NEXT 5=DONE
    // ================================================================
    integer diag_n;
    initial diag_n = 0;
    always @(posedge clk) begin
        if (rstn && (u_dut.u_conv.cstate != 3'd0) && (diag_n < 80)) begin
            $display("[DIAG] t=%0t st=%0d acc=%0d col=%0d row=%0d | re=%b o_col=%0d o_acc=%0d o_roff=%0d | ifm00=%h | macvd=%b pixvd=%b pix=%h",
                $time,
                u_dut.u_conv.cstate, u_dut.u_conv.acc_cyc,
                u_dut.u_conv.col_idx, u_dut.u_conv.row_idx,
                u_dut.conv_ifm_re,  u_dut.conv_ifm_col,
                u_dut.conv_ifm_acc, u_dut.conv_ifm_row_off,
                u_dut.ifm_00[31:0],
                u_dut.u_conv.mac_vld_d, u_dut.conv_pixel_vld, u_dut.conv_pixel);
            diag_n = diag_n + 1;
        end
    end
