`timescale 1ns / 1ns
//----------------------------------------------------------------------
// l14_verify_tb.v — L14 (CONV1x1, Ci=512 Co=195, 8×8) 2-단계 검증 TB
//
//   [Phase A] Standalone L14
//     - L14 IFM (= L13 OFM = CONV14_input.hex, NCHW byte) 을 NHWC entry
//       packed 으로 DRAM L14 IFM 영역에 사전 적재 (software REPACK)
//     - wgt/bias .mem 적재 (L14 영역 포함)
//     - layer_idx=14, conv_phase_r=14, state_r=S_LOAD_BIAS 강제 진입 → release
//     - L14 conv 단독 실행 → layer_idx=15
//     - DRAM L14 OFM ↔ CONV14_output.hex 비교
//
//   [Phase B] Chain L0 → ... → L14 (RTL L13→L14 REPACK)
//     - 시뮬레이션 리셋 → wgt/bias/ifm .mem 적재 → ap_start=1
//     - 자연 진행 (L0~L13 + L13→L14 RTL REPACK + L14 conv)
//     - DRAM L14 OFM ↔ CONV14_output.hex 비교
//
// DRAM 메모리 맵 (L14):
//   L13 OFM           : 0x00308000  (32KB = 8192 word, conv 2×2 packed)
//   L14 IFM (REPACK)  : 0x00310000  (32KB = 8192 word, NHWC entry)
//   L14 OFM           : 0x00318000  (~12.5KB = 3120 word, conv 2×2 packed)
//                       Co=195, 195 × 16 word = 3120 word
//----------------------------------------------------------------------
`include "user_define_h.v"

module l14_verify_tb;

`ifdef FPGA
    initial begin
        $display("[L14V-TB][FATAL] FPGA macro enabled.");
        $finish;
    end
`else
    ...
    parameter L14_IFM_HEX  = "../../../../../../testbench/inout_data_sw/log_feamap/CONV14_input.hex";
    parameter L14_OFM_HEX  = "../../../../../../testbench/inout_data_sw/log_feamap/CONV14_output.hex";
    ...
    localparam integer L14_IFM_OFF_W    = 32'h00310000 >> 2;
    localparam integer L14_OFM_OFF_W    = 32'h00318000 >> 2;
    localparam integer L14_CO_INT       = 195;
    ...
    // Golden buffers
    reg [7:0] golden_l14_ifm [0:32767];  // 512 × 8 × 8 (L14 input = L13 OFM, NCHW)
    reg [7:0] golden_l14_ofm [0:12479];  // 195 × 8 × 8 (L14 OFM, NCHW)

    integer mm_l14_A, mm_l14_B;   // phase 별 total mismatch
    integer tf_l14_A, tf_l14_B;   // phase 별 |delta| > TOLERANCE mismatch
    ...
    task load_dram_inputs;
        begin
            $display("[L14V-TB] Loading WGT  : %s", WGT_MEM);
            $readmemh(WGT_MEM,  dram, 0,              WGT_TOTAL_WORDS - 1);
            $display("[L14V-TB] Loading BIAS : %s", BIAS_MEM);
            $readmemh(BIAS_MEM, dram, BIAS_WORD_BASE, BIAS_WORD_BASE + BIAS_TOTAL_WORDS - 1);
            $display("[L14V-TB] Loading IFM  : %s (L0 IFM)", IFM_MEM);
    ...
    task load_goldens;
        begin
            $display("[L14V-TB] Loading L14 IFM golden : %s", L14_IFM_HEX);
            $readmemh(L14_IFM_HEX, golden_l14_ifm);
            $display("[L14V-TB] Loading L14 OFM golden : %s", L14_OFM_HEX);
            $readmemh(L14_OFM_HEX, golden_l14_ofm);
        end
    endtask

    // [Phase A] Software REPACK: golden_l14_ifm (NCHW byte, 32768) → DRAM L14 IFM (NHWC entry)
    //   동일 식으로 L12 의 변환과 같음 (Ci=512, ci_g 0..127, col_b 0..1, row 0..7)
    //   entry_idx = row*256 + ci_g*2 + col_b (8192 entries total)
    task software_repack_l14_ifm;
        integer t_row, t_cig, t_cb, t_col_l, t_ch_l;
        integer t_addr_w, t_word_idx;
        reg [7:0] tb [0:15];
        reg [31:0] tword;
        begin
            $display("[L14V-TB] Software REPACK : L14 IFM (NHWC entry packing, 8192 word)...");
                    ...
                    for (t_cb = 0; t_cb < 2; t_cb = t_cb + 1) begin
                        for (t_col_l = 0; t_col_l < 4; t_col_l = t_col_l + 1) begin
                            for (t_ch_l = 0; t_ch_l < 4; t_ch_l = t_ch_l + 1) begin
                                tb[t_col_l*4 + t_ch_l] =
                                    golden_l14_ifm[(t_cig*4+t_ch_l)*64 + t_row*8 + (t_cb*4+t_col_l)];
                            end
                        end
                        for (t_word_idx = 0; t_word_idx < 4; t_word_idx = t_word_idx + 1) begin
                            ...
                            t_addr_w = OFM_WORD_BASE + L14_IFM_OFF_W +
            ...
            $display("[L14V-TB] Software REPACK complete (8192 word at L14 IFM region)");
        end
    endtask

    // L14 OFM 비교 + Δ magnitude 분포
    //   L14 OFM 형식 = conv 2×2 packed
    //   word @ (fi, h_block, w_block), fi 0..194 (Co=195)
    //   DRAM word addr = OFM_WORD_BASE + L14_OFM_OFF_W + fi*16 + h_block*4 + w_block
    task compare_l14_ofm;
        ...
        begin
            ...
            cnt_pos = 0; cnt_neg = 0;
            max_diff = 0;
            for (t_fi = 0; t_fi < L14_CO_INT; t_fi = t_fi + 1) begin
                for (t_hb = 0; t_hb < 4; t_hb = t_hb + 1) begin
                    for (t_wb = 0; t_wb < 4; t_wb = t_wb + 1) begin
                        tw = dram[OFM_WORD_BASE + L14_OFM_OFF_W + t_fi*16 + t_hb*4 + t_wb];
                        for (t_sh = 0; t_sh < 2; t_sh = t_sh + 1) begin
                            for (t_sw = 0; t_sw < 2; t_sw = t_sw + 1) begin
                                t_idx = t_sh*2 + t_sw;
                                tg = tw[t_idx*8 +: 8];
                                te = golden_l14_ofm[t_fi*64 + (t_hb*2+t_sh)*8 + (t_wb*2+t_sw)];
                                if (tg !== te) begin
                                    ...
                                    if (t_print < 8) begin
                                        $display("  [L14V-TB] MISMATCH fi=%0d h=%0d w=%0d got=%02x exp=%02x (d=%0d)",
            ...
            $display("[L14V-TB] delta dist : total=%0d  +d=%0d  -d=%0d  max|d|=%0d",
                     mismatch_cnt, cnt_pos, cnt_neg, max_diff);
            $display("[L14V-TB]   |d|=1    : %0d  (%0d%%)",
                     cnt_d1, (cnt_d1*100)/((mismatch_cnt==0)?1:mismatch_cnt));
            $display("[L14V-TB]   |d|=2    : %0d", cnt_d2);
            $display("[L14V-TB]   |d|=3    : %0d", cnt_d3);
            $display("[L14V-TB]   |d|=4..8 : %0d", cnt_d4_8);
            $display("[L14V-TB]   |d|>8    : %0d", cnt_d_big);
            $display("[L14V-TB]   |d|>%0d (tol-exceed) : %0d", TOLERANCE, tol_fail_cnt);
        end
    endtask
    ...
    initial begin
        ...
        //==================================================================
        // [Phase A] Standalone L14 (1×1)
        //==================================================================
        $display("");
        $display("[L14V-TB] ============== Phase A : Standalone L14 ==============");

        load_dram_inputs;
        software_repack_l14_ifm;
        ...
        @(posedge clk);
        force u_yolo_engine.layer_idx    = 5'd14;
        force u_yolo_engine.conv_phase_r = 5'd14;
        force u_yolo_engine.fi_r         = 12'd0;
        force u_yolo_engine.rb_r         = 12'd0;
        force u_yolo_engine.state_r      = 6'd1;        // S_LOAD_BIAS
        $display("[L14V-TB][%0t] Phase A : force layer_idx=14 conv_phase=14 state=S_LOAD_BIAS", $time);
        @(posedge clk);
        ...
        wait (u_yolo_engine.layer_idx == 5'd15);
        $display("[L14V-TB][%0t] Phase A : L14 COMPLETED (layer_idx -> 15)", $time);
        #(40*CLK_PERIOD);

        compare_l14_ofm(mm_l14_A, tf_l14_A);
        $display("[L14V-TB][Phase A] OFM mismatch: %0d / %0d   (tol-exceed: %0d)", mm_l14_A, L14_CO_INT*64, tf_l14_A);
        if      (mm_l14_A == 0) $display("[L14V-TB][Phase A] *** PASS (exact, 0 mismatches) ***");
        else if (tf_l14_A == 0) $display("[L14V-TB][Phase A] *** PASS (within tolerance +-%0d, %0d noise) ***", TOLERANCE, mm_l14_A);
        else                    $display("[L14V-TB][Phase A] *** FAIL (%0d exceed +-%0d tolerance) ***", tf_l14_A, TOLERANCE);

        //==================================================================
        // [Phase B] Chain L0 → ... → L14
        //==================================================================
        $display("");
        $display("[L14V-TB] ============== Phase B : Chain L0 -> ... -> L14 ==============");
        ...
        $display("[L14V-TB][%0t] Phase B : ap_start = 1", $time);
        @(posedge clk); @(posedge clk);
        force u_yolo_engine.u_axi.slv_reg0 = 32'd0;

        wait (u_yolo_engine.layer_idx == 5'd15);
        $display("[L14V-TB][%0t] Phase B : L0 -> ... -> L14 ALL COMPLETED", $time);
        #(40*CLK_PERIOD);

        compare_l14_ofm(mm_l14_B, tf_l14_B);
        $display("[L14V-TB][Phase B] OFM mismatch: %0d / %0d   (tol-exceed: %0d)", mm_l14_B, L14_CO_INT*64, tf_l14_B);
        if      (mm_l14_B == 0) $display("[L14V-TB][Phase B] *** PASS (exact, 0 mismatches) ***");
        else if (tf_l14_B == 0) $display("[L14V-TB][Phase B] *** PASS (within tolerance +-%0d, %0d noise) ***", TOLERANCE, mm_l14_B);
        else                    $display("[L14V-TB][Phase B] *** FAIL (%0d exceed +-%0d tolerance) ***", tf_l14_B, TOLERANCE);

        $display("");
        $display("[L14V-TB] ============================================================");
        $display("[L14V-TB] Phase A : %s (mismatch=%0d, tol-exceed=%0d)",
                 (tf_l14_A == 0) ? "PASS" : "FAIL", mm_l14_A, tf_l14_A);
        $display("[L14V-TB] Phase B : %s (mismatch=%0d, tol-exceed=%0d)",
                 (tf_l14_B == 0) ? "PASS" : "FAIL", mm_l14_B, tf_l14_B);
        $display("[L14V-TB] ============================================================");
    ...
    initial begin
        repeat(400_000) #(1_000_000);
        $display("[L14V-TB] *** TIMEOUT ***");
        $finish;
    end
    ...
    always @(posedge clk) begin
        if (rstn && u_yolo_engine.layer_idx !== prev_li) begin
            $display("[L14V-TB][%0t] >>> layer_idx %0d -> %0d <<<",
