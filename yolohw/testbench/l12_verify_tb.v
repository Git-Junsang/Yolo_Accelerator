...
`else
    ...
    task load_dram_inputs;
        begin
            ...
            $display("[L12V-TB] Loading IFM  : %s (L0 IFM — chain 검증용)", IFM_MEM);
    ...
    task compare_l12_ofm;
        ...
        begin
                        ...
                        for (t_sh = 0; t_sh < 2; t_sh = t_sh + 1) begin
                            for (t_sw = 0; t_sw < 2; t_sw = t_sw + 1) begin
                                ...
                                if (tg !== te) begin
                                    ...
                                    if (t_print < 8) begin
                                        $display("  [L12 MISMATCH] fi=%0d h=%0d w=%0d got=%02x exp=%02x (d=%0d)",
                                                 t_fi, t_hb*2+t_sh, t_wb*2+t_sw, tg, te, diff_signed);
                                        t_print = t_print + 1;
                                    end
                                    if (abs_diff > 3 && cnt_d4_8 + cnt_d_big <= 16) begin
                                        $display("  [L12 MISMATCH BIG-Δ] fi=%0d h=%0d w=%0d got=%02x exp=%02x (d=%0d)",
                                                 t_fi, t_hb*2+t_sh, t_wb*2+t_sw, tg, te, diff_signed);
                                    end
                                end
                            end
            ...
            $display("[L12V-TB] Δ 분포 — total=%0d, +Δ=%0d, -Δ=%0d, max|Δ|=%0d",
                     mismatch_cnt, cnt_pos, cnt_neg, max_diff);
            $display("[L12V-TB]   |Δ|=1 : %0d   (%0d%%)",
                     cnt_d1, (cnt_d1*100)/((mismatch_cnt==0)?1:mismatch_cnt));
            $display("[L12V-TB]   |Δ|=2 : %0d", cnt_d2);
            $display("[L12V-TB]   |Δ|=3 : %0d", cnt_d3);
            $display("[L12V-TB]   |Δ|=4..8 : %0d", cnt_d4_8);
            $display("[L12V-TB]   |Δ|>8 : %0d", cnt_d_big);
        end
    endtask
    ...
    initial begin
        ...
        $display("");
        $display("[L12V-TB] ============== Phase A : Standalone L12 (1×1 mode) ==============");
        ...
        compare_l12_ofm(mm_l12_A);
        $display("[L12V-TB][Phase A] L12 OFM mismatch: %0d / 16,384", mm_l12_A);
        if (mm_l12_A == 0) $display("[L12V-TB][Phase A] *** PASS *** : L12 1×1 conv 단독 OK");
        else               $display("[L12V-TB][Phase A] *** FAIL ***");

        //==================================================================
        // [Phase B] Chain L0 → L1 → ... → L11 → L12 (RTL REPACK 통한 자연 진입)
        //==================================================================
        $display("");
        $display("[L12V-TB] ============== Phase B : Chain L0 → ... → L12 (RTL REPACK) ==============");
        ...
        compare_l12_ofm(mm_l12_B);
        $display("[L12V-TB][Phase B] L12 OFM mismatch: %0d / 16,384", mm_l12_B);
        if (mm_l12_B == 0) $display("[L12V-TB][Phase B] *** PASS ***");
        else               $display("[L12V-TB][Phase B] *** FAIL *** (또는 propagation 누적)");

        $display("");
        $display("[L12V-TB] ============================================================");
        $display("[L12V-TB] Phase A (standalone L12 1×1) : %s (mismatch=%0d)",
                 (mm_l12_A == 0) ? "PASS" : "FAIL", mm_l12_A);
        $display("[L12V-TB] Phase B (L0→...→L12 chain)   : %s (mismatch=%0d)",
                 (mm_l12_B == 0) ? "PASS" : "FAIL", mm_l12_B);
        $display("[L12V-TB] ============================================================");
