...
`else
    ...
    integer mm_l18_A, mm_l18_B;   // phase 별 total mismatch
    integer tf_l18_A, tf_l18_B;   // phase 별 |delta| > TOLERANCE mismatch
    ...
    task software_pack_l17_ofm;
        ...
        integer t_addr_w;
        begin
            $display("[L18V-TB] Software pack : L17 OFM (NCHW -> conv 2x2 packed, 2048 word)...");
    ...
    task compare_l18_ofm;
        ...
        begin
                        ...
                        for (t_sh = 0; t_sh < 2; t_sh = t_sh + 1) begin
                            for (t_sw = 0; t_sw < 2; t_sw = t_sw + 1) begin
                                ...
                                if (tg !== te) begin
                                    ...
                                    if (t_print < 8) begin
                                        $display("  [L18V-TB] MISMATCH f=%0d h=%0d w=%0d got=%02x exp=%02x (d=%0d)",
            ...
            $display("[L18V-TB] delta dist : total=%0d  +d=%0d  -d=%0d  max|d|=%0d",
                     mismatch_cnt, cnt_pos, cnt_neg, max_diff);
            $display("[L18V-TB]   |d|=1    : %0d  (%0d%%)",
                     cnt_d1, (cnt_d1*100)/((mismatch_cnt==0)?1:mismatch_cnt));
            $display("[L18V-TB]   |d|=2    : %0d", cnt_d2);
            $display("[L18V-TB]   |d|=3    : %0d", cnt_d3);
            $display("[L18V-TB]   |d|=4..8 : %0d", cnt_d4_8);
            $display("[L18V-TB]   |d|>8    : %0d", cnt_d_big);
            $display("[L18V-TB]   |d|>%0d (tol-exceed) : %0d", TOLERANCE, tol_fail_cnt);
        end
    endtask
    ...
    initial begin
        ...
        //==================================================================
        $display("");
        $display("[L18V-TB] ============== Phase A : Standalone L18 ==============");
        ...
        compare_l18_ofm(mm_l18_A, tf_l18_A);
        $display("[L18V-TB][Phase A] OFM mismatch: %0d / 32768   (tol-exceed: %0d)", mm_l18_A, tf_l18_A);
        if      (mm_l18_A == 0) $display("[L18V-TB][Phase A] *** PASS (exact, 0 mismatches) ***");
        else if (tf_l18_A == 0) $display("[L18V-TB][Phase A] *** PASS (within tolerance +-%0d, %0d noise) ***", TOLERANCE, mm_l18_A);
        else                    $display("[L18V-TB][Phase A] *** FAIL (%0d exceed +-%0d tolerance) ***", tf_l18_A, TOLERANCE);
        ...
        compare_l18_ofm(mm_l18_B, tf_l18_B);
        $display("[L18V-TB][Phase B] OFM mismatch: %0d / 32768   (tol-exceed: %0d)", mm_l18_B, tf_l18_B);
        if      (mm_l18_B == 0) $display("[L18V-TB][Phase B] *** PASS (exact, 0 mismatches) ***");
        else if (tf_l18_B == 0) $display("[L18V-TB][Phase B] *** PASS (within tolerance +-%0d, %0d noise) ***", TOLERANCE, mm_l18_B);
        else                    $display("[L18V-TB][Phase B] *** FAIL (%0d exceed +-%0d tolerance) ***", tf_l18_B, TOLERANCE);

        $display("");
        $display("[L18V-TB] ============================================================");
        $display("[L18V-TB] Phase A : %s (mismatch=%0d, tol-exceed=%0d)",
                 (tf_l18_A == 0) ? "PASS" : "FAIL", mm_l18_A, tf_l18_A);
        $display("[L18V-TB] Phase B : %s (mismatch=%0d, tol-exceed=%0d)",
                 (tf_l18_B == 0) ? "PASS" : "FAIL", mm_l18_B, tf_l18_B);
        $display("[L18V-TB] ============================================================");
