...
module l13_verify_tb;

`ifdef FPGA
    initial begin
        $display("[L13V-TB][FATAL] FPGA macro enabled.");
        $finish;
    end
`else
    ...
    task load_dram_inputs;
        begin
            $display("[L13V-TB] Loading WGT  : %s", WGT_MEM);
            $readmemh(WGT_MEM,  dram, 0,              WGT_TOTAL_WORDS - 1);
            $display("[L13V-TB] Loading BIAS : %s", BIAS_MEM);
            $readmemh(BIAS_MEM, dram, BIAS_WORD_BASE, BIAS_WORD_BASE + BIAS_TOTAL_WORDS - 1);
            $display("[L13V-TB] Loading IFM  : %s (L0 IFM)", IFM_MEM);
    ...
    task load_goldens;
        begin
            $display("[L13V-TB] Loading L13 IFM golden : %s", L13_IFM_HEX);
            $readmemh(L13_IFM_HEX, golden_l13_ifm);
            $display("[L13V-TB] Loading L13 OFM golden : %s", L13_OFM_HEX);
    ...
    task software_repack_l13_ifm;
        ...
        reg [31:0] tword;
        begin
            $display("[L13V-TB] Software REPACK : L13 IFM (NHWC entry packing, 4096 word)...");
            ...
            $display("[L13V-TB] Software REPACK complete (4096 word at L13 IFM region)");
        end
    endtask
    ...
    initial begin
        ...
        $display("[L13V-TB][%0t] Phase A : force layer_idx=13 conv_phase=13 state=S_LOAD_BIAS", $time);
        @(posedge clk);
        ...
        $display("[L13V-TB][%0t] Phase B : ap_start = 1", $time);
