`timescale 1ns / 1ns
//----------------------------------------------------------------------
// l13_verify_tb.v — L13 (CONV3x3, Ci=256 Co=512, 8×8) 2-단계 검증 TB
//
//   [Phase A] Standalone L13
//     - L13 IFM (= L12 OFM = CONV13_input.hex, NCHW byte) 을 NHWC entry packed
//       으로 DRAM L13 IFM 영역에 사전 적재 (software REPACK)
//     - wgt/bias .mem 적재 (L13 영역 포함)
//     - layer_idx=13, conv_phase_r=13, state_r=S_LOAD_BIAS 강제 진입 → release
//     - L13 conv 단독 실행 → layer_idx=14
//     - DRAM L13 OFM ↔ CONV13_output.hex 비교
//
//   [Phase B] Chain L0 → ... → L13 (RTL L12→L13 REPACK)
//     - 시뮬레이션 리셋 → wgt/bias/ifm .mem 적재 → ap_start=1
//     - 자연 진행 (L0~L12 + L12→L13 RTL REPACK + L13 conv)
//     - DRAM L13 OFM ↔ CONV13_output.hex 비교
//
// DRAM 메모리 맵 (L13):
//   L12 OFM           : 0x00300000  (16KB = 4096 word, conv 2×2 packed)
//   L13 IFM (REPACK)  : 0x00304000  (16KB = 4096 word, NHWC entry)
...
`ifdef FPGA
    initial begin
        $display("[L13DIAG][FATAL] FPGA macro enabled.");
        $finish;
    end
`else
    ...
    parameter L13_IFM_HEX  = "../../../../../../testbench/inout_data_sw/log_feamap/CONV13_input.hex";
    parameter L13_OFM_HEX  = "../../../../../../testbench/inout_data_sw/log_feamap/CONV13_output.hex";
    ...
    task load_dram_inputs;
        begin
            $display("[L13DIAG] Loading WGT  : %s", WGT_MEM);
            $readmemh(WGT_MEM,  dram, 0,              WGT_TOTAL_WORDS - 1);
            $display("[L13DIAG] Loading BIAS : %s", BIAS_MEM);
            $readmemh(BIAS_MEM, dram, BIAS_WORD_BASE, BIAS_WORD_BASE + BIAS_TOTAL_WORDS - 1);
            $display("[L13DIAG] Loading IFM  : %s (L0 IFM)", IFM_MEM);
    ...
    task load_goldens;
        begin
            $display("[L13DIAG] Loading L13 IFM golden : %s", L13_IFM_HEX);
            $readmemh(L13_IFM_HEX, golden_l13_ifm);
            $display("[L13DIAG] Loading L13 OFM golden : %s", L13_OFM_HEX);
            $readmemh(L13_OFM_HEX, golden_l13_ofm);
        end
    endtask

    // [Phase A] Software REPACK: golden_l13_ifm (NCHW byte) → DRAM L13 IFM (NHWC entry)
    //   L13 IFM 한 entry (16 byte) = 4 col × 4 ch byte-interleaved
    //     byte (col_l*4 + ch_l) = golden_l13_ifm[(ci_g*4+ch_l)*64 + row*8 + (col_b*4+col_l)]
    //   DRAM word addr = OFM_WORD_BASE + L13_IFM_OFF_W +
    //                    (row*128 + ci_g*2 + col_b)*4 + word_in_entry
    //   L13: ci_g 0..63 → 64 ci_g, col_b 0..1 → entries per row = 128
    task software_repack_l13_ifm;
        integer t_row, t_cig, t_cb, t_col_l, t_ch_l;
        integer t_addr_w, t_word_idx;
        reg [7:0] tb [0:15];
        reg [31:0] tword;
        begin
            $display("[L13DIAG] Software REPACK : L13 IFM (NHWC entry packing, 4096 word)...");
                    ...
                    for (t_cb = 0; t_cb < 2; t_cb = t_cb + 1) begin
                        for (t_col_l = 0; t_col_l < 4; t_col_l = t_col_l + 1) begin
                            for (t_ch_l = 0; t_ch_l < 4; t_ch_l = t_ch_l + 1) begin
                                tb[t_col_l*4 + t_ch_l] =
                                    golden_l13_ifm[(t_cig*4+t_ch_l)*64 + t_row*8 + (t_cb*4+t_col_l)];
                            end
                        end
                        for (t_word_idx = 0; t_word_idx < 4; t_word_idx = t_word_idx + 1) begin
                            ...
                            t_addr_w = OFM_WORD_BASE + L13_IFM_OFF_W +
            ...
            $display("[L13DIAG] Software REPACK complete (4096 word at L13 IFM region)");
        end
    endtask

    // L13 OFM 비교 + Δ magnitude 분포
    //   L13 OFM 형식 = conv 2×2 packed
    ...
    task compare_l13_ofm;
        ...
        begin
            ...
            for (t_fi = 0; t_fi < 512; t_fi = t_fi + 1) begin
                for (t_hb = 0; t_hb < 4; t_hb = t_hb + 1) begin
                    for (t_wb = 0; t_wb < 4; t_wb = t_wb + 1) begin
                        tw = dram[OFM_WORD_BASE + L13_OFM_OFF_W + t_fi*16 + t_hb*4 + t_wb];
                        for (t_sh = 0; t_sh < 2; t_sh = t_sh + 1) begin
                            for (t_sw = 0; t_sw < 2; t_sw = t_sw + 1) begin
                                t_idx = t_sh*2 + t_sw;
                                tg = tw[t_idx*8 +: 8];
                                te = golden_l13_ofm[t_fi*64 + (t_hb*2+t_sh)*8 + (t_wb*2+t_sw)];
                                if (tg !== te) begin
                                    ...
                                    if (t_print < 8) begin
                                        $display("  [L13DIAG] MISMATCH fi=%0d h=%0d w=%0d got=%02x exp=%02x (d=%0d)",
            ...
            $display("[L13DIAG] delta dist : total=%0d  +d=%0d  -d=%0d  max|d|=%0d",
                     mismatch_cnt, cnt_pos, cnt_neg, max_diff);
            $display("[L13DIAG]   |d|=1    : %0d  (%0d%%)",
                     cnt_d1, (cnt_d1*100)/((mismatch_cnt==0)?1:mismatch_cnt));
            $display("[L13DIAG]   |d|=2    : %0d", cnt_d2);
            $display("[L13DIAG]   |d|=3    : %0d", cnt_d3);
            $display("[L13DIAG]   |d|=4..8 : %0d", cnt_d4_8);
            $display("[L13DIAG]   |d|>8    : %0d", cnt_d_big);
            $display("[L13DIAG]   |d|>%0d (tol-exceed) : %0d", TOLERANCE, tol_fail_cnt);
        end
    endtask
    ...
    initial begin
        ...
        //==================================================================
        $display("");
        $display("[L13DIAG] ============== Phase A : Standalone L13 ==============");
        ...
        $display("[L13DIAG][%0t] Phase A : force layer_idx=13 conv_phase=13 state=S_LOAD_BIAS", $time);
        @(posedge clk);
        ...
        $display("[L13DIAG][%0t] Phase A : L13 COMPLETED (layer_idx -> 14)", $time);
        #(40*CLK_PERIOD);

        compare_l13_ofm(mm_l13_A, tf_l13_A);
        $display("[L13DIAG][Phase A] OFM mismatch: %0d / 32768   (tol-exceed: %0d)", mm_l13_A, tf_l13_A);
        if      (mm_l13_A == 0) $display("[L13DIAG][Phase A] *** PASS (exact, 0 mismatches) ***");
        else if (tf_l13_A == 0) $display("[L13DIAG][Phase A] *** PASS (within tolerance +-%0d, %0d noise) ***", TOLERANCE, mm_l13_A);
        else                    $display("[L13DIAG][Phase A] *** FAIL (%0d exceed +-%0d tolerance) ***", tf_l13_A, TOLERANCE);

        // [L13DIAG] 진단 목적: Phase A(standalone, acc_len=64)만으로 acc/col/row look-ahead
        //   정렬을 다 볼 수 있으므로 Phase B(chain) 는 skip (시뮬 시간 절약).
        $display("[L13DIAG] Phase A done -- diag finish (Phase B chain skipped)");
        $finish;
        ...
        //==================================================================
        $display("");
        $display("[L13DIAG] ============== Phase B : Chain L0 -> ... -> L13 ==============");
        ...
        $display("[L13DIAG][%0t] Phase B : ap_start = 1", $time);
        @(posedge clk); @(posedge clk);
        force u_yolo_engine.u_axi.slv_reg0 = 32'd0;

        wait (u_yolo_engine.layer_idx == 5'd14);
        $display("[L13DIAG][%0t] Phase B : L0 -> ... -> L13 ALL COMPLETED", $time);
        #(40*CLK_PERIOD);

        compare_l13_ofm(mm_l13_B, tf_l13_B);
        $display("[L13DIAG][Phase B] OFM mismatch: %0d / 32768   (tol-exceed: %0d)", mm_l13_B, tf_l13_B);
        if      (mm_l13_B == 0) $display("[L13DIAG][Phase B] *** PASS (exact, 0 mismatches) ***");
        else if (tf_l13_B == 0) $display("[L13DIAG][Phase B] *** PASS (within tolerance +-%0d, %0d noise) ***", TOLERANCE, mm_l13_B);
        else                    $display("[L13DIAG][Phase B] *** FAIL (%0d exceed +-%0d tolerance) ***", tf_l13_B, TOLERANCE);

        $display("");
        $display("[L13DIAG] ============================================================");
        $display("[L13DIAG] Phase A : %s (mismatch=%0d, tol-exceed=%0d)",
                 (tf_l13_A == 0) ? "PASS" : "FAIL", mm_l13_A, tf_l13_A);
        $display("[L13DIAG] Phase B : %s (mismatch=%0d, tol-exceed=%0d)",
    ...
    always @(posedge clk) begin
        if (rstn && u_yolo_engine.layer_idx !== prev_li) begin
            $display("[L13DIAG][%0t] >>> layer_idx %0d -> %0d <<<",
    ...
    // ================================================================
    //  conv 정렬 진단 trace (L13: acc_len=64 → acc/col/row look-ahead 전부)
    //    발사 주소(o_ifm_*) → mac_vld_d → pixel_vld 의 cycle 정렬 기록.
    //    L0(acc_len=1)와 달리 o_acc 0→63 진행이 보임 → B 의 acc look-ahead 검증.
    //    cstate: 0=IDLE 1=LOAD 2=RUN 3=DRAIN 4=NEXT 5=DONE
    // ================================================================
    integer diag_n;
    initial diag_n = 0;
    always @(posedge clk) begin
        if (rstn && (u_yolo_engine.u_conv.cstate != 3'd0) && (diag_n < 140)) begin
            $display("[DIAG] t=%0t st=%0d acc=%0d col=%0d row=%0d | re=%b o_col=%0d o_acc=%0d o_roff=%0d | macvd=%b pixvd=%b pix=%h",
                $time,
                u_yolo_engine.u_conv.cstate, u_yolo_engine.u_conv.acc_cyc,
                u_yolo_engine.u_conv.col_idx, u_yolo_engine.u_conv.row_idx,
                u_yolo_engine.conv_ifm_re,  u_yolo_engine.conv_ifm_col,
                u_yolo_engine.conv_ifm_acc, u_yolo_engine.conv_ifm_row_off,
                u_yolo_engine.u_conv.mac_vld_d, u_yolo_engine.conv_pixel_vld, u_yolo_engine.conv_pixel);
            diag_n = diag_n + 1;
        end
    end
