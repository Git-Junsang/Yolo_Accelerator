...
`else
    ...
    task compare_l17_ofm;
        ...
        begin
                        ...
                        for (t_sh = 0; t_sh < 2; t_sh = t_sh + 1) begin
                            for (t_sw = 0; t_sw < 2; t_sw = t_sw + 1) begin
                                ...
                                if (tg !== te) begin
                                    ...
                                    if (t_print < 8) begin
                                        $display("  [L17 MISMATCH] fi=%0d h=%0d w=%0d got=%02x exp=%02x (d=%0d)",
                                                 t_fi, t_hb*2+t_sh, t_wb*2+t_sw, tg, te, diff_signed);
                                        t_print = t_print + 1;
                                    end
                                    if (abs_diff > 3 && cnt_d4_8 + cnt_d_big <= 16) begin
                                        $display("  [L17 MISMATCH BIG-Δ] fi=%0d h=%0d w=%0d got=%02x exp=%02x (d=%0d)",
                                                 t_fi, t_hb*2+t_sh, t_wb*2+t_sw, tg, te, diff_signed);
                                    end
                                end
                            end
            ...
            $display("[L17V-TB] Δ 분포 — total=%0d, +Δ=%0d, -Δ=%0d, max|Δ|=%0d",
                     mismatch_cnt, cnt_pos, cnt_neg, max_diff);
            $display("[L17V-TB]   |Δ|=1 : %0d   (%0d%%)",
                     cnt_d1, (cnt_d1*100)/((mismatch_cnt==0)?1:mismatch_cnt));
            $display("[L17V-TB]   |Δ|=2 : %0d", cnt_d2);
            $display("[L17V-TB]   |Δ|=3 : %0d", cnt_d3);
            $display("[L17V-TB]   |Δ|=4..8 : %0d", cnt_d4_8);
            $display("[L17V-TB]   |Δ|>8 : %0d", cnt_d_big);
        end
    endtask
    ...
    initial begin
        ...
        //==================================================================
        $display("");
        $display("[L17V-TB] ============== Phase A : Standalone L17 (1×1) ==============");
        ...
        compare_l17_ofm(mm_l17_A);
        $display("[L17V-TB][Phase A] L17 OFM mismatch: %0d / 8,192", mm_l17_A);
        if (mm_l17_A == 0) $display("[L17V-TB][Phase A] *** PASS *** : L17 1×1 conv 단독 OK");
        else               $display("[L17V-TB][Phase A] *** FAIL ***");
        ...
        compare_l17_ofm(mm_l17_B);
        $display("[L17V-TB][Phase B] L17 OFM mismatch: %0d / 8,192", mm_l17_B);
        if (mm_l17_B == 0) $display("[L17V-TB][Phase B] *** PASS ***");
        else               $display("[L17V-TB][Phase B] *** FAIL *** (또는 propagation 누적)");

        $display("");
        $display("[L17V-TB] ============================================================");
        $display("[L17V-TB] Phase A (standalone L17 1×1) : %s (mismatch=%0d)",
                 (mm_l17_A == 0) ? "PASS" : "FAIL", mm_l17_A);
        $display("[L17V-TB] Phase B (L0→...→L17)         : %s (mismatch=%0d)",
                 (mm_l17_B == 0) ? "PASS" : "FAIL", mm_l17_B);
        $display("[L17V-TB] ============================================================");
