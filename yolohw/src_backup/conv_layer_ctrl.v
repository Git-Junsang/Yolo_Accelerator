...
module conv_layer_ctrl(
    ...
    //----------------------------------------------------------
    output reg [10:0]   wgt_addr,       // weight BRAM 주소
    ...
    //----------------------------------------------------------
    output reg [16:0]   ofm_wr_addr,    // 출력 FM 쓰기 주소
...
//
// weight 시작 주소 (원본 RTL 의 base 값을 그대로 유지).
//   L0: 0,  L1: 432,  L2: 1440
//   ※ 호스트 hex (skeleton/bin/log_param/CONV*_param_weight.hex) 는
//     1 byte / line tight packing (L0 = 16×27 = 432 byte) 이며 zero-padding 없음.
//     이 컨트롤러는 강의 정통 경로(cnn_ctrl + mac_kern) 도입 시 교체 예정이므로
//     param_wgt_base 의 단위 / 의미는 추후 mac_kern 통합 시 재정의한다.
//
// bias 시작 주소 (원본 RTL 값 유지):
//   L0: 0,  L1: 16,  L2: 48
//   ※ 호스트 hex 는 2 byte / line (INT16 big-endian 16진수) 이며
//     L0 = 16 entry = 32 byte. RTL 측 sign-extend 후 32-bit 저장.
//----------------------------------------------------------------------

// 레이어별 파라미터를 reg에 래치 (FSM에서 레이어 전환 시 설정)
reg [8:0]  param_out_h;         // 출력 높이
reg [8:0]  param_out_w;         // 출력 폭
reg [5:0]  param_co;            // 출력 채널 수 (최대 64)
reg [4:0]  param_num_cycles;    // MAC 누적 사이클 수
reg [4:0]  param_shift_amount;  // descaling 시프트량
reg [10:0] param_wgt_base;      // weight 시작 주소
reg [6:0]  param_bias_base;     // bias 시작 주소
reg [8:0]  param_in_h;          // 입력 높이
reg [8:0]  param_in_w;          // 입력 폭
reg [5:0]  param_ci;            // 입력 채널 수
...
localparam  ST_IDLE         = 4'd0,     // 대기
            ...
            ST_LAYER_DONE   = 4'd10,    // 레이어 완료 → 다음 레이어 또는 종료
            ST_DONE         = 4'd11;    // 전체 완료
...
reg [5:0]  out_fil;         // 현재 필터 인덱스 (0 ~ Co-1)
    ...
    case (state)
        ...
        ST_LAYER_DONE: begin
            if (current_layer == 2'd2)
                next_state = ST_DONE;           // Layer 2 끝 → 전체 완료
            else
                next_state = ST_LAYER_SETUP;    // 다음 레이어
        end
            ...
            ST_LAYER_SETUP: begin
                ...
                case (current_layer)
                    2'd0: begin // CONV00: 256×256×3 → 256×256×16
                        ...
                        param_shift_amount <= 5'd7;  // log2(128) TODO: 실제 값 확인 필요
                        param_wgt_base  <= 11'd0;
                        param_bias_base <= 7'd0;
                    end
                    2'd1: begin // CONV02: 128×128×16 → 128×128×32
                        ...
                        param_shift_amount <= 5'd6;  // log2(64) TODO: 실제 값 확인 필요
                        param_wgt_base  <= 11'd432;  // L0 weight 끝 (16 filter × 27 byte)
                    ...
                    2'd2: begin // CONV04: 64×64×32 → 64×64×64
                        ...
                        param_shift_amount <= 5'd6;  // log2(64) TODO: 실제 값 확인 필요
                        param_wgt_base  <= 11'd1440; // 원본 RTL base 값 유지
